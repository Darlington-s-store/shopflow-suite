...
export const addToCart = async (req, res) => {
  try {
    ...
    // Fetch full details for the added item to return to frontend
    const enrichedResult = await pool.query(
      `SELECT ci.*, p.name as product_name, p.slug, p.base_price, pv.color, pv.storage, pv.price as variant_price,
    ...
    res.json({ success: true, item: enrichedResult.rows[0] });
  } catch (error) {
